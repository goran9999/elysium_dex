 ...
 * @category Quotes
 * @param pool - the account data for the pool this position belongs to
...
export function collectRewardsQuote(param: CollectRewardsQuoteParam): CollectRewardsQuote {
  const { pool, position, tickLower, tickUpper, timeStampInSeconds } = param;

  const { tickCurrentIndex, rewardInfos: poolRewardsInfos, rewardLastUpdatedTimestamp } = pool;
  ...
  for (let i = 0; i < NUM_REWARDS; i++) {
    ...
    const rewardInfo = poolRewardsInfos[i];
    const positionRewardInfo = positionRewardInfos[i];
    invariant(!!rewardInfo, "poolRewardsInfos cannot be undefined");
    ...
    // Increment the global reward growth tracker based on time elasped since the last pool update.
